...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const response = await fetch("./assets/data/survey.json");
  ...
  startButton.addEventListener("click", () => {
    survey_id = Date.now();  // Generates a unique timestamp
    ...
    console.log(`✅ Survey with an ID ${survey_id} started for ${selectedAnimal}.`);
...
async function showResults() {
  ...
  surveyContainer.innerHTML = `
      ...
      <p>The data from your survey has been saved to the Curious database, and a PDF report has been e-mailed to ${recipientEmail}.</p>
  ...
  const fileName = `Welfare_survey-${survey_id}.pdf`;
  ...
  console.log("📤 Submitting welfare survey with file URL:", fileUrl);
  
  // ✅ Ensure `fileUrl` is correctly passed
  await submitVisitorSurvey(fileUrl);  // ✅ No need to pass fileUrl as an argument
  }
...
async function uploadToR2(pdfBlob, fileName) {
  ...
  try {
    const response = await fetch("https://get-curio.us/api/visitor-survey-reports/upload", {
      method: "POST",
      body: formData,
    });
...
async function exportResults() {
  ...
  doc.setFontSize(16);
  doc.text(`Visitor Survey`, 15, 20);
  doc.setFontSize(12);
  doc.text(`Survey ID: ${survey_id} - ${selectedAnimal}`, 15, 30);
  ...
  try {
    ...
    const uploadResponse = await fetch("https://get-curio.us/api/visitor-survey-reports/upload", {
      method: "POST",
      body: formData
    });
    ...
    const emailPayload = {
      ...
      subject: `🔔 New Visitor Survey: ${selectedAnimal}`,
      message: `A welfare survey (ID ${survey_id}) for ${selectedAnimal} has just been completed by ${document.getElementById("auditor-name").value}!\n\nDownload the report here: ${fileLink}`
    };

    const emailResponse = await fetch("https://get-curio.us/api/visitor-survey-reports/send-email", {
      method: "POST",
...
async function submitVisitorSurvey(fileUrl) {
  console.log("📤 Sending welfare survey to database...");
  ...
  if (!fileUrl) {
    console.error("❌ Error: fileUrl is missing! Cannot submit survey.");
    return;
}
...
console.log("📄 Payload being sent:", surveyData);

  try {
      const response = await fetch("https://get-curio.us/api/save-visitor-surveys/save", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      console.log("✅ Welfare survey saved to database successfully!");
  } catch (error) {
      console.error("❌ Failed to save survey to database:", error);
  }
}
...
async function getSiteEmail() {
  try {
    ...
    if (!configData.surveysEmail) {
      throw new Error("❌ surveysEmail is missing in config.json.");
    }

    console.log("✅ Retrieved email:", configData.surveysEmail);
    return configData.surveysEmail;
  } catch (error) {
...
function downloadReport() {
  ...
  const zip = new JSZip();
  zip.file(`Welfare_survey-${survey_id}.pdf`, reportBlob);
  zip.file(`Welfare_survey-${survey_id}.csv`, csvBlob);

  zip.generateAsync({ type: "blob" }).then((zipBlob) => {
    const zipUrl = URL.createObjectURL(zipBlob);
    const downloadAnchor = document.createElement("a");
    downloadAnchor.href = zipUrl;
    downloadAnchor.download = `Curious-WA-${survey_id}.zip`;
